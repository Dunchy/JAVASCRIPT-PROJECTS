...
function remove() {
    /* This retrieves the id of the clicked button, which matches its array index */
    var id = this.getAttribute('id');
    var todos = get_todos();
    
    /* This removes that specific item at that index */
    todos.splice(id, 1);
    
    /* This updates local storage with the modified array */
    localStorage.setItem('todo', JSON.stringify(todos));
    
    /* This re-renders the changed list back to the screen */
    show();
...
function show() {
    ...
    /* Adds a click event listener to the X buttons */
